...
function Parties() {
  ...
  return (
    <div className="App">
      ...
      <Button variant="contained" size='large' margin={5} color="secondary" onClick={() => changeView(Hobby)} >Parties</Button>
      </Stack>
      ...
      <Grid 
          ...
          > 
      {Hobby.map((mb) => (<Grid sx={{ maxWidth: 300, minHeight: 300, margin: 4, border: '5px outset black', borderRadius: 5, backgroundColor: "white", }} item xs={12} sm={6} md={3} key={mb.id}>{mb.img}<br/>Name:{mb.name}<br/>Price:{mb.price}<br/> 
